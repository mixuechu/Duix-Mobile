...
// 🚀 新增状态管理属性
@property (nonatomic, assign) GJLWebSocketConnectionState connectionState;
@property (nonatomic, strong) NSMutableDictionary<NSString *, GJLWebSocketStateChangeHandler> *stateChangeHandlers;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        // 🚀 初始化状态管理
        _connectionState = GJLWebSocketConnectionStateDisconnected;
        _stateChangeHandlers = [NSMutableDictionary dictionary];
...
+ (BOOL)isConnectionReady {
    return [self currentConnectionState] == GJLWebSocketConnectionStateConnected;
}
...
+ (void)addStateChangeHandler:(GJLWebSocketStateChangeHandler)handler withIdentifier:(NSString *)identifier {
    if (handler && identifier) {
        [[self sharedManager].stateChangeHandlers setObject:[handler copy] forKey:identifier];
    }
}

+ (void)removeStateChangeHandlerWithIdentifier:(NSString *)identifier {
    if (identifier) {
        [[self sharedManager].stateChangeHandlers removeObjectForKey:identifier];
    }
}

// 🚀 状态变化通知
- (void)_updateConnectionState:(GJLWebSocketConnectionState)newState error:(NSError *)error {
    dispatch_async(self.wsQueue, ^{
        if (self.connectionState != newState) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                NSLog(@"🚀 [WS] State changed to: %ld", (long)newState);
                for (NSString *identifier in self.stateChangeHandlers.allKeys) {
...
- (void)preConnectToURL:(NSString *)urlString {
    [self preConnectToURL:urlString completion:nil];
}
...
- (void)_connectToURL:(NSString *)urlString completion:(GJLWebSocketConnectCompletion)completion {
    ...
    // 🚀 更新连接状态为正在连接
    [self _updateConnectionState:GJLWebSocketConnectionStateConnecting error:nil];
...
- (void)URLSession:(NSURLSession *)session webSocketTask:(NSURLSessionWebSocketTask *)webSocketTask didOpenWithProtocol:(NSString *)protocol {
    dispatch_async(self.wsQueue, ^{
        ...
        // 🚀 更新连接状态为已连接
        [self _updateConnectionState:GJLWebSocketConnectionStateConnected error:nil];
...
- (void)URLSession:(NSURLSession *)session webSocketTask:(NSURLSessionWebSocketTask *)webSocketTask didCloseWithCode:(NSURLSessionWebSocketCloseCode)closeCode reason:(NSData *)reason {
    dispatch_async(self.wsQueue, ^{
        ...
        // 🚀 更新连接状态为断开
        NSError *closeError = [NSError errorWithDomain:@"GJLWebSocketManager" 
                                                 code:closeCode 
                                             userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"WebSocket closed with code: %ld", (long)closeCode]}];
        [self _updateConnectionState:GJLWebSocketConnectionStateDisconnected error:closeError];
