...
async def main():
    ...
    headers = {
        ...
        "X-Api-Resource-Id": (
            resource_id if resource_id else get_resource_id(voice_type)
        ),
    ...
    logger.info(
        # 修改此处：将 response.headers 改为 response_headers
        f"Connected to WebSocket server, Logid: {websocket.response_headers['x-tt-logid']}",
    )
        ...
        for i, sentence in enumerate(sentences):
            ...
            while True:
                ...
                if msg.type == MsgType.FullServerResponse:
                    ...
                    end_time = time.time()
                    logger.info(f"Time cost: {end_time - begin_time} seconds")
                else:
            ...
            if audio_data:
                filename = f"{voice_type}_session_{i}.{encoding}"
