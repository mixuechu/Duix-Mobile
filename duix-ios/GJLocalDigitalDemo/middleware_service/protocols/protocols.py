...
class Message:
    ...
    def __str__(self) -> str:
        ...
        if self.type in [MsgType.AudioOnlyServer, MsgType.AudioOnlyClient]:
            ...
            # 安全处理payload解码
            payload_str = self.payload.decode('utf-8', 'ignore') if isinstance(self.payload, bytes) else str(self.payload)
            return f"MsgType: {self.type}, EventType:{self.event}, ErrorCode: {self.error_code}, Payload: {payload_str}"
        else:
            # 安全处理payload解码
            payload_str = self.payload.decode('utf-8', 'ignore') if isinstance(self.payload, bytes) else str(self.payload)
            if self.flag in [MsgTypeFlagBits.PositiveSeq, MsgTypeFlagBits.NegativeSeq]:
                return f"MsgType: {self.type}, EventType:{self.event}, Sequence: {self.sequence}, Payload: {payload_str}"
